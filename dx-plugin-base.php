 ...
 * Description: Add a hover button to any Word Press image to enable your users to buy prints and photo gifts sucha as magnets, frames, mousepads and more. Print Money pays site owners up to 85% of markup.
	...
	public function __construct() {
        ...
        function pluginname_ajaxurl() {
            ...
            <script type="text/javascript">
                ...
                var currentPage = '<?php echo get_the_ID(); ?>';
                var currentCat = '<?php $categories= get_the_category(get_the_ID()); echo $categories[0]->cat_ID; ?>';
                var currentUserRole = '<?php echo  get_user_role(); ?>';
            </script>
        <?php
        }
        ...
        function click()
        {
            ...
            $explodehrefs = explode("&", $explodehref[1]);

            $pdg = $wpdb->get_row("SELECT * FROM $wpdb->posts WHERE post_content = '" . $explodehrefs[0]. "' && post_excerpt = '".$explodehref[3]."'", 'ARRAY_A');

            if($pdg > 0) {
                $my_post = array(
                    'ID' => $pdg['ID'],
                    'post_title' => 'New Click',
                    'post_content' => $explodehrefs[0],
                    'post_excerpt' => $explodehref[3],
                    'post_type' => 'img-print-clicks',
                    'post_status' => 'publish',
                    'post_parent' => $pdg['post_parent'] + 1
        ...
        function fetch_data() {
            ...
            $ex = explode(",", $_GET['postType']);
$arr = array();
            foreach ($ex as $v) {

                $postType = $v;

                $outputQuery = $wpdb->get_results("SELECT * FROM $wpdb->posts WHERE post_status = 'publish' AND post_type = '".$v."'");

                $i = 0;
                foreach ( $outputQuery as $fivesdraft )
                {
                    $arr[$v][$i] = $fivesdraft->post_content;
	...
	public function dx_add_admin_CSS( $hook ) {
		...
		if( 'toplevel_page_dx-plugin-base' === $hook ) {
			...
			wp_register_style('dx_help_page',  plugins_url( '/imageprintsadmin.css', __FILE__ ) );
	...
	public function dx_admin_pages_callback() {
		...
		add_menu_page(__( "Print Money", 'dxbase' ), __( "Print Money", 'dxbase' ), 'edit_themes', 'dx-plugin-base', array( $this, 'dx_plugin_base' ) );		
	...
	public function dx_plugin_base() {
		...
		include_once( DXP_PATH . '/imageprints.php' );
	...
	public function dx_custom_post_types_callback() {
		...
		register_post_type( 'pluginbase', array(
			...
			'labels' => array(
				...
				'add_new' => _x("Block", 'pluginbase', 'dxbase' ),
	...
	public function dx_register_settings() {
		...
		require_once( DXP_PATH . '/imageprintsclass.php' );
